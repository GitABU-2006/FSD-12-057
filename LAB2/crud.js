    ...
    const total = data.reduce((t,item) => t + item.price * item.qty, 0);
    console.log(`you have to pay total amount:` , total);
    };

    const remvoveFromCart = async (pid) => {
    const data = await getCart();
    ...
    if (isFound) {
        isFound.qty = value;
        await saveCart(data);
        console.log(`product quantity updated with id ${pid}`);
    } else {
        console.log(`product with id ${pid} not found`);
    }
    };
    ...
    do {
        ...
        switch (Number(choice)) {
          ...
          case 3:
            ...
            await remvoveFromCart(Number(pid));
            break;
          case 4:
            let pid1 = await cin.question("Enter product id to update: ");
            let value = await cin.question("+1 increase or -1 decrease");
            await updateCart(Number(pid1), Number(value));
            break;
          case 5:
